...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        tempList = inActs[:]
        tempList.insert(0,1)
        return (self.sigmoid(self.getWeightedSum(tempList)))
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        derivative = (1+exp(value)) * (1+exp(value))
        return ((exp(value))/(derivative))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        tempList = inActs[:]
        tempList.insert(0,1)
        return ((self.sigmoidDeriv(self.getWeightedSum(tempList))))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(tempList)):
        	...
        	updateW = self.weights[i] + (alpha * tempList[i] * delta)
        	self.weights[i] = updateW
        	modification = updateW - oldWeight
        	emptyList.append(abs(modification))
        totalModification = sum(emptyList)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        tempList = inActs[:]
        finalList = []
        finalList.insert(0,tempList)

        for i in range(self.numHiddenLayers):
        	layer1 = []
        	for h in self.hiddenLayers[i]:
        		layer1.append(h.sigmoidActivation(tempList[:]))
        	finalList.append(layer1)
        	tempList = layer1[:]
        outputList = []

        for l in self.outputLayer:
        	outputList.append(l.sigmoidActivation(tempList[:]))
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    tempVal = 0
                    for node in xrange(len(nextLayer)):
                    	newDelta = nextLayer[node].weights[neuronNum + 1] * deltas[0][node]
                    	tempVal = tempVal +  newDelta 
                    delta = gPrime * tempVal
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for exampleIn, exampleOut in examplesTest:
    	output = nnet.feedForward(exampleIn)[-1]
    	match = True   
    	for valueInd in xrange(len(output)):
    		if round(output[valueInd]) != exampleOut[valueInd]:
    			match = False
    	if match == False:
    		testError += 1
    	else:
    ...
    denom = (testCorrect + testError) * 1.0
    testAccuracy = testCorrect/denom
