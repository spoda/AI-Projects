...
class GreedyBustersAgent(BustersAgent):
	...
	def chooseAction(self, gameState):
		...
		ghostsDict  = []
		counter = 0	
		for lg in livingGhosts:
			for lgp in livingGhostPositionDistributions:
				newBelief  = 0
				if (counter != 0):
					counter += 1
				for (pos, belief) in lgp.items():
					if (belief > newBelief):
						newBelief  = belief
						ghostsDict.insert(counter, pos)
		temp = 0
		closestDist = -1
		for val in ghostsDict:
			ghostDistance = self.distancer.getDistance(pacmanPosition, val)
			if (closestDist == -1):
				closestDist = ghostDistance
				temp = val
			elif (closestDist > ghostDistance):
				closestDist = ghostDistance
				temp = val
		for x in legal:
			newDist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, x), temp)
