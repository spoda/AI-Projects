    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                eModelDist = emissionModel[trueDistance]
                newBeliefs = self.beliefs[p]
                allPossible[p] = eModelDist * newBeliefs
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPostDist.items():
                newDist = prob * self.beliefs[p]
                ghosts[newPos] += newDist
        ghosts.normalize()
        self.beliefs = ghosts
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        pNum = self.numParticles
        pBeliefs = util.Counter()
        if noisyDistance == None:
            self.particles = []
            ...
            for p in self.particles:
                ...
                pBeliefs[p] += emissionModel[distance]
            if pBeliefs.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for i in range(self.numParticles):
                    self.particles[i] = util.sample(pBeliefs)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        counter = 0
        iterList = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
            ...
            for p in self.particles:
                ...
                for g in range(self.numGhosts):
                    if noisyDistances[g] == None:
                        ...
                        newDist = util.manhattanDistance(p[g], pacmanPosition)
                        counter *= emissionModels[g][newDist]
                jpBeliefs[p] += counter

            if (jpBeliefs.totalCount() != 0):
                jpBeliefs.normalize()
                temp1 = []
                for n in range(self.numParticles):
                    temp1.append(util.sample(jpBeliefs))
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for g in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), g, self.ghostAgents[g])
